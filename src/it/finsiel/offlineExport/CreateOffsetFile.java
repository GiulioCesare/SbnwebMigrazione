...
//		else if (dataFilenameIn.contains("tr_des_des"))
		...
		else if (dataFilenameIn.indexOf("tr_luo_luo") != -1)
			inTrLuoLuo = true;
...
//	else if (args[0].indexOf("tr_idsbn_idaltri") != -1 )
	else if ((args[0].indexOf("tr_idsbn_idaltri") != -1) && !(args[0].indexOf(".rel") != -1))
	{
...
void findOffsetsTrLuoLuo() 
{
	...
	for (int i = 0; i < bytesRead; i++) {
		...
		if (buffer1[i] == '\n') {
			...
			if (listaInvertita)
				lidBase = sb.substring(13, 23);
			
			else
				lidBase = sb.substring(0, 10);
			
			
			if (!lastDidBase.equals(lidBase)) {
				scriviOffset(offsetBufferredWriter1, lidBase, Long.toString(lastOffset));
				lastLidBase = lidBase;
			}
