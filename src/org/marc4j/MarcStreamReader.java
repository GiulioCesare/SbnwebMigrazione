// $Id: MarcStreamReader.java,v 1.1 2013/09/06 10:50:01 argentino Exp $
/**
...
//bytesRead = input.read(CrLf);
	 ...
	 * Returns the next record in the iteration.
	 *
	 * 21/09/2009 16.22 Arge, gestione BUG export MIL. LF all'interno dei dati
	 * 
	 * @return Record - the record object
	 */
	public Record nextLfInDati_Arge() {
		Leader ldr;
		int bytesRead = 0;
		...
		try {
			...
			// Cicliamo sui campi unimarc (tags)
			for (int i = 0; i < size; i++) {
				// Prendi il tag (l'etichetta)
				bytesRead = input.read(tag);
				
				// Prendi la lunghezza del tag
				while (bytesRead != -1 && bytesRead != tag.length)
				...
				tmp = new String(length);
				lengths[i] = Integer.parseInt(tmp);
				// Prendi la posizione iniziale dei dati	
				bytesRead = input.read(start);
			...
			for (int i = 0; i < size; i++) {
				if (Verifier.isControlField(tags[i])) {
					...
					try {
						...
						throw new MarcException( "error parsing data field for tag: " + tags[i]);
//										+ " with data: "
//										+ new String(byteArray), e);
					}
				}
