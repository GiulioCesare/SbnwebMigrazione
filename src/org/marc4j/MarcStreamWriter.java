// $Id: MarcStreamWriter.java,v 1.1 2013/09/06 10:50:01 argentino Exp $
/**
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                while (si.hasNext()) {
                    ...
                    byte[] ba = getDataElement(sf.getData()); 
                    data.write(ba);
                }
                data.write(Constants.FT);
// ARGE 11/01/2008
//                dir.write(getEntry(df.getTag(), data.size() - previous, previous));
                byte [] directoryEntry = getEntry(df.getTag(), data.size() - previous, previous);
    			if ((directoryEntry.length % 12) != 0)
    				throw new MarcException("invalid directory length: " + directoryEntry.length);
                dir.write(directoryEntry);
// End Arge 
                previous = data.size();
            }
    ...
    private byte[] getDataElement(String data) throws IOException {
        if (converter != null)
        {
//        	String s = converter.convert(data); 
//        	byte [] ba = s.getBytes(encoding);
//            return ba;

        	byte [] ba = converter.convertToBytes(data);
            return ba;
